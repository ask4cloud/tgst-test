...
class TransitGateway(object):
    ...
    def _create_tag_local(self, resource, key, message):
        try:
            self.logger.info("Executing: " + self.__class__.__name__ + "/" + inspect.stack()[0][3])
            ec2 = EC2(self.logger, environ.get('AWS_REGION'))
            ec2.create_tags(resource, key, message )
        except Exception as e:
            message = self._message(inspect.stack()[0][3], e)
            self.logger.exception(message)            
        ...
        try:
            ...
            if self.event.get('TgwAttachmentExist').lower() == 'yes':
                ...
                if response[0].get('State') == 'pending' or response[0].get('State') == 'modifying':
                    ...
                    sleep(_seconds)
                # ODP - Tagging Hub TGW Attachments with all associated CIDR ranges.
                # Assumes VpcCidrRanges was updated in the VPC._describe_vpc method.
                if response[0].get('State') == 'available':
                    self._print('ODP - Trying to tag...', response ) 
                    self._create_tag_local(self.event.get('TransitGatewayAttachmentId'), 'VpcCidrRanges', self.event.get('VpcCidrAll') )    
            else:
...
class VPC(object):
    ...
    def _describe_vpc(self):
        try:
            ...
            # Start - ODP Updates for maintaining all CIDR ranges event
            # Get all CIDR ranges in the VPC and update the event.

            cidr_list = []
            # IPv4
            for cidr in vpc.get('CidrBlockAssociationSet'):
              if cidr['CidrBlockState']['State'] == 'associated':
                cidr_list.append(cidr['CidrBlock'])
            # IPv6
            if 'Ipv6CidrBlockAssociationSet' in vpc:
              for cidr in vpc.get('Ipv6CidrBlockAssociationSet'):
                if cidr['Ipv6CidrBlockState']['State'] == 'associated':
                  cidr_list.append(cidr['Ipv6CidrBlock'])

            cidr_all = ", "
            cidr_all = cidr_all.join(cidr_list)
            # Replace with update to new event key VpcCidrAll
            print('CIDR_LIST: ', cidr_all ) 
            self.event.update({'VpcCidrAll': cidr_all }) 
            # Cidr block associated with this VPC
            self.event.update({'VpcCidr': vpc.get('CidrBlock')})
           
            # End - ODP Updates for maintaining all CIDR ranges in event
